...
AddEventHandler('wasabi_boombox:saveSong', function()
    ...
    if input[1] and input[2] then
        ...
        TriggerClientEvent("dopeNotify:SendNotification", source, {		
            text = '<b><i class="fas fa-bell"></i>&nbsp;&nbsp;POWIADOMIENIE</span></b></br>Zapisano!',
            type = "ogolne",
            timeout = 5000,
            layout = "topRight"
        })
        ...
        TriggerClientEvent("dopeNotify:SendNotification", source, {		
            text = '<b><i class="fas fa-bell"></i>&nbsp;&nbsp;POWIADOMIENIE</span></b></br>Błędny link lub fraza!',
            type = "ogolne",
            timeout = 5000,
            layout = "topRight"
        })
...
AddEventHandler('wasabi_boombox:deleteSong', function(data)
	local confirmed = lib.alertDialog({
		header = 'Usuń Piosenke',
		content = 'Czy chcesz usunąć tą piosenke?',
		centered = true,
		cancel = true
	})
	if confirmed == 'confirm' then
        ...
        TriggerClientEvent("dopeNotify:SendNotification", source, {		
            text = '<b><i class="fas fa-bell"></i>&nbsp;&nbsp;POWIADOMIENIE</span></b></br>Piosenka usunięta!',
            type = "ogolne",
            timeout = 5000,
            layout = "topRight"
        })
        ...
        TriggerClientEvent("dopeNotify:SendNotification", source, {		
            text = '<b><i class="fas fa-bell"></i>&nbsp;&nbsp;POWIADOMIENIE</span></b></br>Anulowano!',
            type = "ogolne",
            timeout = 5000,
            layout = "topRight"
        })
...
AddEventHandler('wasabi_boombox:playMenu', function(data)
    ...
    if data.type == 'play' then
        local keyboard = lib.inputDialog('Włącz Muzyke', {'Youtube URL','Dystans (Max 40)', 'Głośność (1-100)'})
        if keyboard then
    ...
    elseif data.type == 'volume' then
        local keyboard = lib.inputDialog('Zmień Głośność', {'Wartość (1-100)'})    
        if keyboard then
    ...
    elseif data.type == 'distance' then
        local keyboard = lib.inputDialog('Zmień Dystans', {'Wartość (Max 40)'})
        if keyboard then
